...
//            throw new IllegalArgumentException("Content '" + researchName + "' isn't in the tech tree, but '" + unlock.name + "' requires it to be researched.");
    ...
    private static void addException(Exception exception) {
        ErrorContent key = new ErrorContent();
        key.minfo.mod = modInfo;
        key.minfo.sourceFile = new Fi("byte-logic-error.error");
        key.minfo.error = Strings.getStackTrace(exception);
        modInfo.erroredContent.add(key);
    }
