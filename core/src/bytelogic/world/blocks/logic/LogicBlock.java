...
import arc.util.*;
import arc.util.io.*;
import arclibrary.utils.events.EventReceiver;
import arclibrary.utils.events.EventSender;
import arclibrary.utils.events.EventSignature;
import bytelogic.content.*;
import bytelogic.core.*;
import bytelogic.event.ByteLogicTapEvent;
...
//    public abstract int signal(Tile tile);
    public static final EventSender<Void> blLogicTileTapped = new EventSender<>("bl-logic-tile-tapped",
        EventSignature.make("build", Building.class)
    );
    @Load("@baseName()")
...
//        controllable = false;
    ...
    public void setBars() {
        ...
        addBar("signal", (Building e) -> {
            ...
            return new Bar(
                () -> Core.bundle.format("block.signal", entity.displaySignal()),
                () -> {
                    return entity.displaySignal().barColor();
                },
                () -> 1f);
        });
    }
    ...
    public abstract class LogicBuild extends Building implements ByteLogicBuildingc, CustomSaveBuilding {
        public final Signal displaySignal = new Signal();
        protected final Signal lastSignal = new Signal();
        protected final Signal nextSignal = new Signal();
        protected int index__bytelogicBuild = -1;

        @Override
        public void setBLIndex___byteLogicBuild(int index) {
        ...
        @Override
        public void updateDisplaySignalState() {
            displaySignal.set(lastSignal);
        }
